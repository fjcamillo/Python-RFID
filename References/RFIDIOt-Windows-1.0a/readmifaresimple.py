...
KEYS= ['FFFFFFFFFFFF','A0A1A2A3A4A5','B0B1B2B3B4B5','000000000000','ABCDEF012345']
KEYTYPES=['AA','BB','FF']
DEFAULT_KEYTYPE= 'AA'
BLOCKS_PER_SECT= 4
START_BLOCK= 0
...
card.info('readmifaresimple v0.1f')
...
while True:
	...
	if not reset:
		if keytype == 'AA':
			print '  KeyA will be set to', key + ', KeyB will be set to FFFFFFFFFFFF'
		else:
			print '  KeyA will be set to FFFFFFFFFFFF,', 'KeyB will be set to', key 
	else:
		...
		if (block + 1) % 4:
			...
			if reset:
				...
				if keytype == 'BB':
					...
					# ACL plus KeyB
					blockdata= key + CloneData[outblock][12:]
